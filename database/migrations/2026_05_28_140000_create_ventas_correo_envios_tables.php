...
return new class extends Migration
{
    ...
    {
        if (! Schema::hasTable('ventas_correo_envios')) {
            Schema::create('ventas_correo_envios', function (Blueprint $table) {
                ...
                $table->index(['user_id', 'created_at'], 'vce_user_created_idx');
            });
        }

        if (! Schema::hasTable('ventas_correo_envio_destinatarios')) {
            Schema::create('ventas_correo_envio_destinatarios', function (Blueprint $table) {
                $table->id();
                $table->foreignId('ventas_correo_envio_id')
                    ->constrained('ventas_correo_envios')
                    ->cascadeOnDelete();
                $table->unsignedBigInteger('ventas_correo_destinatario_id')->nullable();
                $table->index('ventas_correo_destinatario_id', 'vced_destinatario_id_idx');
        ...
        if (! Schema::hasTable('ventas_correo_envio_adjuntos')) {
