...
import {
    ventasMainCardClass,
    ventasTableShellClass,
    ventasColumnTitleClass,
    ventasColumnTitleDotClass,
    ventasPageIconWrapClass,
    ventasPageTitleClass,
    ventasPageSubtitleClass,
    ventasTitleAccentBarClass,
    ventasFilterSelectClass,
    ventasFilterInputClass,
} from '@/lib/ventasUi'
...
export default function AdminCotizacionesInvitadoPage() {
    ...
    const columnTitle = (label) => (
        <div className={`${ventasColumnTitleClass(darkMode)} inline-flex items-center`}>
            <span className={ventasColumnTitleDotClass(darkMode)} aria-hidden />
            {label}
        </div>
    )

    const filaModo = (i) =>
        darkMode ? (i % 2 === 0 ? 'bg-tienda-canvas/55' : 'bg-tienda-elevated/40') : i % 2 === 0 ? 'bg-white' : 'bg-orange-50/40'

    /** Bordes visibles tipo cuadrícula (hoja de cálculo) */
    const gridTh = darkMode
        ? 'border border-orange-800/70 bg-tienda-elevated/95 text-orange-200'
    ...
    const gridTdModalTh = darkMode
        ? 'border border-orange-800/70 bg-tienda-elevated/95'
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <div className="flex flex-wrap items-start gap-4">
                <div className={ventasPageIconWrapClass(darkMode)}>
                    <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                </div>
                <div className="min-w-0 flex-1">
                    <h1 className={ventasPageTitleClass(darkMode)}>Cotizaciones por correo de clientes invitados</h1>
                    <div className={ventasTitleAccentBarClass(darkMode)} aria-hidden />
                    <p className={ventasPageSubtitleClass(darkMode)}>
                        Listado de cotizaciones enviadas por correo a quienes no tenían cuenta. Los datos coinciden con el PDF enviado.
                    </p>
                </div>
            </div>

            <div className={`${ventasMainCardClass(darkMode)} relative`}>
                <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b pb-4 border-orange-500/15 dark:border-orange-500/20">
                    <div className="flex flex-wrap items-center gap-3">
                        {columnTitle('Registros')}
                        <span className={`text-sm ${darkMode ? 'text-orange-200/80' : 'text-orange-800/90'}`}>
                            Total: {total} · Página {paginaActual} de {lastPage || 1}
                        </span>
                    </div>
                    <div className="flex items-center gap-2 text-xs">
                        {tableRefreshing && (
                            <span className={`inline-flex items-center gap-1.5 ${darkMode ? 'text-orange-400/90' : 'text-orange-700'}`}>
                                <span className="inline-block h-3.5 w-3.5 animate-spin rounded-full border-2 border-orange-500 border-t-transparent" aria-hidden />
                                Actualizando tabla…
                            </span>
                        )}
                ...
                <div className={`relative ${ventasTableShellClass(darkMode)}`}>
                    {loading && (
                    ...
                    <table className="table-fixed w-full min-w-[800px] text-sm border-collapse border border-orange-900/40 dark:border-orange-800/50">
                        <colgroup>
                            <col className="w-[13%]" />
                            <col className="w-[39%]" />
                            <col className="w-[28%]" />
                            ...
                            <tr>
                                <th className={`px-3 py-2 align-top min-w-0 overflow-hidden ${gridTh}`}>
                                    ...
                                    <input
                                        ...
                                        className={`${ventasFilterInputClass(darkMode, false, 'w-full min-w-0 max-w-full box-border text-xs py-1.5')} ${folioInput.trim() ? filtroConValorCls : ''}`}
                                    />
                                </th>
                                <th className={`px-3 py-2 align-top min-w-0 overflow-hidden ${gridTh}`}>
                                    ...
                                    <select
                                        ...
                                        className={`${ventasFilterSelectClass(filtroEmail ? false : darkMode, false, 'w-full max-w-full text-xs py-1.5 min-w-0')} ${filtroEmail ? filtroConValorCls : ''}`}
                                    >
                                ...
                                <th className={`px-3 py-2 align-top ${gridTh}`}>
                                    <div className="flex flex-row items-end gap-1.5">
                                        <div className="min-w-0 flex-1 max-w-[min(100%,9.25rem)]">
                                            <span className={`block text-[10px] font-semibold uppercase tracking-wide mb-0.5 ${darkMode ? 'text-orange-400/90' : 'text-orange-800'}`}>
                                                Desde
                                            </span>
                                            <input
                                                type="date"
                                                ...
                                                className={`${ventasFilterInputClass(darkMode, false, 'w-full min-w-0 max-w-full text-xs py-1')} ${fechaDesde ? `${filtroConValorCls} admin-date-input-filled` : darkMode ? 'ventas-date-input-dark' : ''}`}
                                            />
                                        </div>
                                        <div className="min-w-0 flex-1 max-w-[min(100%,9.25rem)]">
                                            <span className={`block text-[10px] font-semibold uppercase tracking-wide mb-0.5 ${darkMode ? 'text-orange-400/90' : 'text-orange-800'}`}>
                                            ...
                                            <input
                                                ...
                                                className={`${ventasFilterInputClass(darkMode, false, 'w-full min-w-0 max-w-full text-xs py-1')} ${fechaHasta ? `${filtroConValorCls} admin-date-input-filled` : darkMode ? 'ventas-date-input-dark' : ''}`}
                                            />
                                        </div>
                                    </div>
                                </th>
                                ...
                                lista.map((row, i) => (
                                    <tr
                                        ...
                                        className={`${filaModo(i)} transition-colors ${
                                            darkMode ? 'hover:bg-orange-950/25' : 'hover:bg-orange-50/70'
                                        }`}
                                    >
                                        <td className={`px-3 py-3.5 align-middle tabular-nums font-semibold ${gridTd} ${darkMode ? 'text-orange-300' : 'text-orange-800'}`}>
                                            {row.folio ?? row.id}
                                        </td>
                                            ...
                                            <div className="flex flex-col gap-2 items-start">
                                                <button
                                                    ...
                                                    className={`inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold border shadow-sm transition-colors ${
                                                        darkMode
                                                            ? 'border-orange-600/80 bg-orange-950/30 text-orange-200 hover:bg-orange-900/40'
            ...
            {modalId != null && (
                <>
                    ...
                    >
                        <div className="shrink-0 flex items-center justify-between px-4 py-3 bg-orange-600/25 border-b border-orange-500/30">
                        ...
                        <div className="p-5 sm:p-6 overflow-y-auto flex-1">
                            {(rowModal || detalleModal) && (
                                <div className={`rounded-xl p-4 mb-4 ${darkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-100 border border-gray-200'}`}>
                                    <p className={`text-xs font-semibold uppercase tracking-wider mb-1 ${darkMode ? 'text-orange-400' : 'text-orange-700'}`}>Folio</p>
                                    ...
                                    <p className={`text-xs font-semibold uppercase tracking-wider mt-3 mb-1 ${darkMode ? 'text-orange-400' : 'text-orange-700'}`}>Correo</p>
                                    <p className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>{rowModal?.email ?? detalleModal?.email ?? '—'}</p>
                                    <p className={`text-xs font-semibold uppercase tracking-wider mt-3 mb-1 ${darkMode ? 'text-orange-400' : 'text-orange-700'}`}>Fecha</p>
                                ...
                                <>
                                    <div className={`rounded-xl overflow-hidden ${darkMode ? 'border border-gray-600' : 'border border-gray-300'}`}>
                                        <table className="w-full text-sm border-collapse">
                                            <thead>
                                                <tr>
                                                    <th className={`px-3 py-2.5 text-left text-xs font-bold uppercase ${darkMode ? 'text-orange-200' : 'text-orange-900'} ${gridTdModalTh}`}>Producto</th>
                                                    <th className={`px-3 py-2.5 text-center w-16 text-xs font-bold uppercase ${darkMode ? 'text-orange-200' : 'text-orange-900'} ${gridTdModalTh}`}>Cant.</th>
                                                    <th className={`px-3 py-2.5 text-right w-28 text-xs font-bold uppercase ${darkMode ? 'text-orange-200' : 'text-orange-900'} ${gridTdModalTh}`}>P. unit.</th>
                                                    <th className={`px-3 py-2.5 text-right w-28 text-xs font-bold uppercase ${darkMode ? 'text-orange-200' : 'text-orange-900'} ${gridTdModalTh}`}>Subtotal</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {(detalleModal.items || []).map((it, idx) => (
                                                    <tr key={idx} className={darkMode ? 'bg-tienda-elevated/50' : 'bg-white'}>
                                                        ...
                                                        <td className={`px-3 py-2.5 text-right font-semibold text-orange-500 tabular-nums ${gridTdModal}`}>{formatPrecio(it.subtotal)}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                    ...
                                    <p className={`mt-4 text-right text-lg font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                                        Total: <span className="text-orange-500">{formatPrecio(detalleModal.total)}</span>
                                    </p>
                                </>
                            )}
                        </div>
