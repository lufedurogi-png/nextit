...
export default function AdminProductosManuales() {
    ...
    const inputClass = darkMode ? 'w-full px-4 py-2.5 rounded-lg border border-gray-600 bg-gray-700/80 text-white focus:ring-2 focus:ring-orange-500/40' : 'w-full px-4 py-2.5 rounded-lg border border-gray-300 bg-white text-gray-900 focus:ring-2 focus:ring-orange-500/30'
        ...
        <div className="space-y-8">
            ...
            <div className={`rounded-xl overflow-hidden border shadow-xl ${darkMode ? 'bg-tienda-elevated border-gray-700' : 'bg-white border-gray-200'}`}>
                <div className={`px-5 py-4 ${darkMode ? 'bg-orange-600/25 border-b border-orange-500/30' : 'bg-orange-50 border-b border-orange-200'}`}>
                    <h2 className={`text-lg font-bold ${darkMode ? 'text-orange-200' : 'text-orange-800'}`}>{editingId ? 'Editar producto' : 'Nuevo producto'}</h2>
                </div>
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            ...
                            <div className="md:col-span-2">
                                ...
                                >
                                    <input
                                        ...
                                        className="h-5 w-5 rounded accent-orange-500 focus:ring-2 focus:ring-orange-500/40"
                                    />
                                    <span className="font-medium">Destacado</span>
                                    <svg className={`w-4 h-4 ml-auto opacity-60 ${form.destacado ? 'text-orange-500' : 'text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        ...
                        <button type="button" onClick={() => addSpec('informacion_general')} className={`text-sm ${darkMode ? 'text-orange-400' : 'text-orange-600'}`}>+ Agregar campo</button>
                    </div>
                    ...
                    <div>
                        ...
                        <button type="button" onClick={() => addSpec('especificaciones')} className={`text-sm ${darkMode ? 'text-orange-400' : 'text-orange-600'}`}>+ Agregar especificación</button>
                    </div>

                    {success && <div className={`p-3 rounded ${darkMode ? 'bg-orange-500/20 text-orange-400' : 'bg-orange-50 text-orange-700'}`}>{success}</div>}
                    <InputError messages={errors.general} />
                    <div className="flex gap-3">
                        <Button type="submit" disabled={saving} className={`${darkMode ? '!bg-orange-500 hover:!bg-orange-400' : '!bg-orange-600 hover:!bg-orange-700'} text-white px-6 py-2.5`}>
                                ...
                                {productos.map((p) => (
                                    <tr key={p.id} className={`border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                                        ...
                                        <td className="py-3 px-4">
                                            <span className={`px-2 py-0.5 rounded text-xs ${p.anulado ? 'bg-amber-500/20 text-amber-400' : 'bg-orange-500/20 text-orange-400'}`}>
                                                {p.anulado ? 'Anulado' : 'Activo'}
                                            </span>
                                        </td>
                                        <td className="py-3 px-4">
                                            <div className="flex gap-2">
                                                <button type="button" onClick={() => handleEdit(p)} className={`px-2 py-1 rounded text-xs ${darkMode ? 'bg-orange-500/20 text-orange-400' : 'bg-orange-100 text-orange-700'}`}>Editar</button>
