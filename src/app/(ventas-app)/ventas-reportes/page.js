...
function KpiCard({ darkMode, title, value, prevValue }) {
    ...
    const positive = d >= 0
    const container = `${cardBase} ${darkMode ? 'border-violet-900/40 bg-[#1a1628]/80' : 'border-violet-100 bg-white'}`
    return (
        <div className={container}>
            <p className={`text-xs font-medium uppercase tracking-wide ${darkMode ? 'text-violet-300/80' : 'text-violet-600/80'}`}>{title}</p>
            <p className={`mt-2 text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{value}</p>
            <p className={`mt-1 text-xs ${darkMode ? 'text-violet-200/60' : 'text-gray-500'}`}>Periodo anterior: {prevValue}</p>
...
export default function VentasReportesPage() {
    ...
    const panel = `${cardBase} ${darkMode ? 'border-violet-900/40 bg-[#1a1628]/80' : 'border-violet-100 bg-white'}`

    return (
        <div className="space-y-6">
            <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                    <h1 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-violet-950'}`}>Reporte comercial operativo</h1>
                        ...
                        <button
                            ...
                            className={`rounded-full px-4 py-1.5 text-xs font-semibold transition ${
                                period === key
                                    ...
                                    : darkMode
                                      ? 'border border-violet-700 text-violet-200 hover:bg-violet-900/40'
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="grid gap-4 xl:grid-cols-2">
                        <div className={panel}>
                            <h2 className={`mb-3 text-sm font-semibold uppercase tracking-wide ${darkMode ? 'text-violet-200' : 'text-violet-900'}`}>Top clientes por monto</h2>
                            <div className="space-y-2">
                                {topClientes.length === 0 ? (
                                    <p className={darkMode ? 'text-violet-300/70' : 'text-gray-500'}>Sin datos en el periodo.</p>
                                ) : (
                                    paginatedClientes.map((c) => (
                                        <div key={c.key} className={`rounded-xl border px-3 py-2 ${darkMode ? 'border-violet-900/40' : 'border-violet-100'}`}>
                                            <div className="flex items-center justify-between gap-3">
                                                <p className={`truncate text-sm font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{c.name}</p>
                                                <p className="text-sm font-bold text-violet-500">{fmtMoney(c.monto)}</p>
                                            </div>
                                            <p className={`text-xs ${darkMode ? 'text-violet-300/70' : 'text-gray-500'}`}>{c.cotizaciones} cotizaciones</p>
                                        </div>
                                    ))
                                )}
                            </div>
                        ...
                        <div className={panel}>
                            <h2 className={`mb-3 text-sm font-semibold uppercase tracking-wide ${darkMode ? 'text-violet-200' : 'text-violet-900'}`}>Top productos cotizados</h2>
                            <div className="space-y-2">
                                {topProductos.length === 0 ? (
                                    <p className={darkMode ? 'text-violet-300/70' : 'text-gray-500'}>Sin datos en el periodo.</p>
                                ) : (
                                    paginatedProductos.map((p) => (
                                        <div key={p.clave} className={`rounded-xl border px-3 py-2 ${darkMode ? 'border-violet-900/40' : 'border-violet-100'}`}>
                                            <div className="flex items-center justify-between gap-3">
                                                <p className={`truncate text-sm font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>{p.nombre}</p>
                                                <p className="text-sm font-bold text-violet-500">{fmtMoney(p.monto)}</p>
                                            </div>
                                            <p className={`text-xs ${darkMode ? 'text-violet-300/70' : 'text-gray-500'}`}>
                    ...
                    <div className={panel}>
                        <div className="mb-3 flex items-center justify-between gap-2">
                            <h2 className={`text-sm font-semibold uppercase tracking-wide ${darkMode ? 'text-violet-200' : 'text-violet-900'}`}>Detalle operativo</h2>
                            <button
                                type="button"
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full text-sm">
                                <thead>
                                    <tr className={darkMode ? 'text-violet-200/80' : 'text-violet-900'}>
                                ...
                                <tbody>
                                    {paginatedRows.length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className={`px-3 py-6 text-center ${darkMode ? 'text-violet-300/70' : 'text-gray-500'}`}>
                                                Sin registros en el periodo.
                                            </td>
                                        </tr>
                                    ) : (
                                        paginatedRows.map((r, idx) => (
                                            <tr key={`${r.folio}-${idx}`} className={`border-t ${darkMode ? 'border-violet-900/30' : 'border-violet-100'}`}>
