...
/** UserType en API (1=Admin, 2=Cliente, 3=Vendedor): no ofertar vendedor en nuevos roles/filtros. */
...
export default function AdminGestionUsuarios() {
    ...
    useEffect(() => {
        if (createType === TIPO_VENDEDOR) setCreateType(2)
    }, [createType])

    useEffect(() => {
        if (filterRole === 'seller') setFilterRole('')
    }, [filterRole])
    ...
    const typesSinVendedor = useMemo(
        () => types.filter((t) => Number(t.id) !== TIPO_VENDEDOR),
        [types]
    )
    const typesForEditSelect = useMemo(
        () => types.filter((t) => Number(t.id) !== TIPO_VENDEDOR || Number(t.id) === Number(editRole)),
        [types, editRole]
    )
    ...
    /**
     * Alineado con `coleccionador-api` → `AdminUserManagementService::passwordRules()`:
     * `Password::min(8)->letters()->mixedCase()->numbers()->symbols()` + `confirmed` en el formulario.
     * Misma regla para crear usuario (admin o cliente) y para reset de contraseña.
     */
    const passwordChecks = {
        minLength: newPassword.length >= 8,
        hasLetters: /\p{L}/u.test(newPassword),
    ...
    const createPasswordChecks = {
        ...
        hasLetters: /\p{L}/u.test(createPassword),
        hasMixedCase: /[a-z]/.test(createPassword) && /[A-Z]/.test(createPassword),
    ...
    const createPasswordRequirements = [
        ...
        { key: 'hasLetters', label: 'Al menos una letra', met: createPasswordChecks.hasLetters },
        { key: 'hasMixedCase', label: 'Mayúscula y minúscula', met: createPasswordChecks.hasMixedCase },
        { key: 'hasNumber', label: 'Al menos un número', met: createPasswordChecks.hasNumber },
        { key: 'hasSymbol', label: 'Al menos un símbolo (!@#$%…)', met: createPasswordChecks.hasSymbol },
    ]
                    ...
                    <form onSubmit={handleCreateUser} className="space-y-5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                ...
                                <div className="relative">
                                    ...
                                    <span className="absolute right-3 top-1/2 -translate-y-1/2">
                                        <PasswordToggleButton
                                            ...
                                            onToggle={() => setShowCreatePassword((s) => !s)}
                                        />
                                    </span>
                                    {showCreatePasswordModal && (
                                        <>
                                            <div className="fixed inset-0 z-40" aria-hidden onClick={() => setShowCreatePasswordModal(false)} />
                                            <div className={`absolute left-full top-1/2 z-50 w-64 max-w-[min(16rem,calc(100vw-2rem))] -translate-y-1/2 ml-1.5 rounded-xl border-2 shadow-xl ${darkMode ? 'border-gray-600 bg-gray-800' : 'border-gray-200 bg-white'}`} role="dialog" aria-labelledby="create-password-requirements-title">
                                                <div className="p-3">
                            ...
                            <div className={`${showCreateConfirmModal ? 'z-[60]' : ''}`}>
                                ...
                                <div className="relative">
                                    ...
                                    <span className="absolute right-3 top-1/2 -translate-y-1/2">
                                        <PasswordToggleButton
                                            visible={showCreatePasswordConfirmation}
                                            onToggle={() => setShowCreatePasswordConfirmation((s) => !s)}
                                            labelShow="Ver confirmación"
                                            labelHide="Ocultar confirmación"
                                        />
                                    </span>
                        ...
                        <div className={`rounded-lg p-4 ${darkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                            ...
                            <div className="relative">
                                ...
                                <span className="absolute right-3 top-1/2 -translate-y-1/2">
                                    <PasswordToggleButton
                                        visible={showCreateAdminPassword}
                                        onToggle={() => setShowCreateAdminPassword((s) => !s)}
                                    />
                                </span>
                            </div>
                                                                        ...
                                                                        <span className="relative inline-block">
                                                                            ...
                                                                            <span className="absolute right-1 top-1/2 -translate-y-1/2">
                                                                                <PasswordToggleButton
                                                                                    ...
                                                                                    onToggle={() => setShowAdminPassword((s) => !s)}
                                                    ...
                                                    {addingRoleUserId === u.id ? (
                                                        <span className="inline-flex flex-wrap items-center gap-1.5">
                                                            ...
                                                            >
                                                                ...
                                                                {types.filter((t) => Number(t.id) !== TIPO_VENDEDOR && !userRolesList(u).includes(tipoToRoleName(t.id))).map((t) => (
                                                                    <option key={t.id} value={t.id}>{t.label}</option>
                                                                ))}
                                                            </select>
                                                            <span className="relative inline-block">
                                                        ...
                                                        types.filter((t) => Number(t.id) !== TIPO_VENDEDOR && !userRolesList(u).includes(tipoToRoleName(t.id))).length > 0 && (
                                                            <button
                                                                type="button"
                                                    ...
                                                    <div className={`relative ${showPasswordModal ? 'z-[60]' : ''}`}>
                                                        ...
                                                        {showPasswordModal && (
                                                            <>
                                                                ...
                                                                <div className={`absolute left-full top-1/2 z-50 w-60 max-w-[min(15rem,calc(100vw-2rem))] -translate-y-1/2 ml-1 rounded-xl border-2 shadow-xl text-xs ${darkMode ? 'border-gray-600 bg-gray-800' : 'border-gray-200 bg-white'}`}>
                                                                    <div className="p-2.5">
                                                    ...
                                                    <div className={`relative ${showConfirmModal ? 'z-[60]' : ''}`}>
                                                        ...
                                                        <span className="absolute right-1 top-1/2 -translate-y-1/2">
                                                            <PasswordToggleButton
                                                                ...
                                                                onToggle={() => setShowNewPasswordConfirmation((s) => !s)}
                                                                iconClass="w-[18px] h-[18px]"
                                                                labelShow="Ver"
                                                                labelHide="Ocultar"
                                                                className="!p-0.5"
