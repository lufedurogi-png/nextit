...
export default function AdminMetodosPagoPage() {
    ...
    const loadMethods = useCallback(async () => {
        ...
        try {
            ...
            setError(e?.response?.data?.message || 'No se pudieron cargar los métodos de pago.')
        } finally {
            setLoading(false)
        }
    }, [])
    ...
    const handleToggle = async (code, current) => {
        ...
        setSavingCode(code)
        try {
            const { data } = await axios.put(`/admin/metodos-pago/${encodeURIComponent(code)}`, {
                active: !current,
            ...
            setError(e?.response?.data?.message || 'Error al guardar en el servidor.')
        } finally {
            setSavingCode(null)
        }
    }
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                                    Activa o desactiva métodos para el checkout del plan Pro (y futuros flujos). Los cambios se guardan en la API.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p className={`text-xs ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                                    Cada cambio requiere la contraseña del administrador (igual que en el proyecto anterior).
                                </p>
                            </div>
                        </div>
                    </div>
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-4">
                                            ...
                                            <div className="flex flex-col items-end gap-2 shrink-0">
                                                ...
                                                {busy && <span className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Aplicando…</span>}
                                            </div>
                                        </div>
                                    </div>
                                )
                    ...
                    <div className={panelShell}>
                        <div className={`px-5 py-3.5 ${panelHead}`}>
                            ...
                            <p className={`mt-1 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                                Confirma con tu contraseña de administrador antes de usar los interruptores.
                            </p>
                        </div>
                        <div className="p-5">
                            ...
                            <div className="relative">
                                ...
                                <button
                                    ...
                                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 text-xs text-gray-500"
                            ...
                            <div className={`mt-3 rounded-lg border-2 px-3 py-2 text-xs ${canToggle
                                ...
                                {canToggle
                                    ? 'Contraseña lista. Puedes activar o desactivar métodos; se valida contra tu cuenta admin.'
                                    : 'Escribe tu contraseña de administrador para habilitar los interruptores.'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={panelShell}>
                        ...
                        <div className="p-5">
                            <ul className={`space-y-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                                <li className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-emerald-500" /> Plan Pro en /planes (modal de métodos de pago)</li>
                                <li className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-amber-500" /> Los clientes solo ven métodos activos</li>
                            </ul>
                        </div>
                    </div>
                </div>
