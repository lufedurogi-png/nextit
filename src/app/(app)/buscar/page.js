...
export default function BuscarPage() {
    ...
    const executeSearch = useCallback(async (term) => {
        ...
        try {
            setLoading(true)
            const { data } = await axios.get('/search/global', { params: { q: t } })
            setResult({
    ...
    useEffect(() => {
        ...
        setQ(t)
        if (t.length >= 2) {
            void executeSearch(t)
        } else {
            setResult({ users: [], groups: [], posts: [], group_posts: [], listings: [] })
        }
    }, [urlQ, executeSearch])
            ...
            <div className="relative z-[1] mx-auto max-w-6xl space-y-3 px-4 pb-12 -mt-3">
                ...
                {activeTab === 'users' ? (
                    <section className="rounded-3xl border border-slate-200 bg-white/90 p-4 dark:border-slate-700 dark:bg-slate-900/70">
                        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Usuarios</p>
                ...
                {activeTab === 'groups' ? (
                    <section className="rounded-3xl border border-slate-200 bg-white/90 p-4 dark:border-slate-700 dark:bg-slate-900/70">
                        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Grupos</p>
                        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                {activeTab === 'posts' ? (
                    <section className="rounded-3xl border border-slate-200 bg-white/90 p-4 dark:border-slate-700 dark:bg-slate-900/70">
                        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Publicaciones</p>
                ...
                {activeTab === 'listings' ? (
                    <section className="rounded-3xl border border-slate-200 bg-white/90 p-4 dark:border-slate-700 dark:bg-slate-900/70">
                        <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Tienda</p>
