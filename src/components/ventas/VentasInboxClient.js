...
const panel =
    'rounded-2xl border border-violet-100 bg-white dark:border-violet-900/40 dark:bg-[#1a1628]/80 flex flex-col overflow-hidden min-h-0 h-full'
    ...
    return (
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
            <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 mb-4">
                <div>
                    <h1 className="text-2xl font-bold text-violet-950 dark:text-white">Chats</h1>
            ...
            <div className="grid flex-1 min-h-0 gap-4 lg:grid-cols-12 lg:grid-rows-1 overflow-hidden">
                <div className={`${panel} lg:col-span-3`}>
                    <div className="shrink-0 border-b border-violet-100 px-3 py-2 dark:border-violet-900/40">
                        <input
                            type="search"
                            value={filtro}
                            onChange={(e) => setFiltro(e.target.value)}
                            placeholder="Filtrar clientes…"
                            className="w-full rounded-lg border border-violet-100 bg-violet-50/50 px-3 py-2 text-sm dark:border-violet-800 dark:bg-[#12101a] dark:text-violet-100"
                        />
                    </div>
                    <ul className="overflow-y-auto flex-1 min-h-0">
                        {loadingClientes ? (
                            <li className="px-3 py-4 text-sm text-violet-600 dark:text-violet-300/70">Cargando…</li>
                        ) : clientesFiltrados.length === 0 ? (
                            <li className="px-3 py-4 text-sm text-violet-600 dark:text-violet-300/70">
                                {clientes.length === 0
                                ...
                                return (
                                    <li key={c.id}>
                                        <button
                                            ...
                                            className={`w-full border-b border-violet-50 px-3 py-3 text-left transition dark:border-violet-900/30 ${
                                                active
                                                    ? 'bg-violet-50/80 dark:bg-violet-600/15'
                                                    : 'hover:bg-violet-50/40 dark:hover:bg-white/[0.03]'
                                            }`}
                                        >
                                            <p className="font-medium text-sm text-gray-900 dark:text-white">
                                                {c.name || 'Sin nombre'}
                                            </p>
                                            <p className="text-xs text-violet-700/70 dark:text-violet-300/60 truncate">
                                                {c.email}
                                            </p>
                ...
                <div className={`${panel} lg:col-span-6`}>
                    <div className="shrink-0 border-b border-violet-100 px-4 py-3 flex items-center justify-between dark:border-violet-900/40">
                        <div>
                ...
                <div className={`${panel} lg:col-span-3`}>
                    <div className="shrink-0 border-b border-violet-100 px-4 py-3 dark:border-violet-900/40">
